...
exports.generateLabelForOrder = generateLabelForOrder;
...
function mergeSettings(prefs) {
    ...
    return {
        ...
        order_info: { ...DEFAULT_LABEL_SETTINGS.order_info, ...(prefs.order_info || {}) },
        shipper_info: {
            ...DEFAULT_LABEL_SETTINGS.shipper_info,
            ...(prefs.shipper_info || {}),
        },
        product_info: {
            ...DEFAULT_LABEL_SETTINGS.product_info,
            ...(prefs.product_info || {}),
        },
        powered_by: prefs.powered_by ?? DEFAULT_LABEL_SETTINGS.powered_by,
    };
}
async function generateLabelForOrder(order, userId, tx = client_1.db) {
    ...
    const settings = mergeSettings(prefs);
    ...
    if (settings.shipper_info?.brandLogo && profileOfUser?.companyInfo?.companyLogoUrl) {
        try {
            const logoUrl = await (0, upload_service_1.presignDownload)(profileOfUser.companyInfo.companyLogoUrl);
            const finalUrl = Array.isArray(logoUrl) ? (logoUrl.length > 0 ? logoUrl[0] : null) : logoUrl;
            if (finalUrl) {
                const logoResp = await axios_1.default.get(finalUrl, {
    ...
    const pickup = typeof order.pickup_details === 'string'
        ? JSON.parse(order.pickup_details || '{}')
        : order.pickup_details ?? {};
    const rto = typeof order.rto_details === 'string'
    ...
    const isEnabled = (value) => (value === undefined ? true : value === true);
    ...
    const barcodeSource = order.barcode_img || order.barcode_url || order.barcode_image || order.barcode || null;
    if (providerKey.includes('delhivery') && barcodeSource) {
        try {
            ...
            if (isValidDataUrl(barcodeSource)) {
                ...
                try {
                    const barcodeResponse = await axios_1.default.get(barcodeSource, {
                        responseType: 'arraybuffer',
                        timeout: 10000,
                    });
                    ...
                    console.warn(`⚠️ Failed to download barcode from URL: ${barcodeSource}`, downloadErr?.message || downloadErr);
                }
            }
            else {
    ...
    const orderBarcode = showOrderBarcode && order.order_number ? await generateBarcodeBase64(order.order_number) : null;
    const invoiceBarcode = showInvoiceBarcode && order.invoice_number
    ...
    const pageContent = [];
    const trimText = (value, max = charLimit) => {
        const text = String(value ?? '').trim();
        if (!text)
            return '-';
        return text.length > max ? `${text.slice(0, max)}...` : text;
    };
    const sellerBrandName = profileOfUser?.companyInfo?.companyName ||
    ...
    const dimensionLabel = order.length && order.breadth && order.height
    ...
    if (includeDeadWeight) {
        ...
        const slabWeightKg = order.charged_slabs && chargeableWeightKg
        ...
        weightLines.push(`Volumetric Weight: ${volumetricWeightKg ? volumetricWeightKg.toFixed(3) + ' kg' : 'calculated'}`);
        weightLines.push(`Chargeable Weight: ${chargeableWeightKg ? chargeableWeightKg.toFixed(3) + ' kg' : '-'}`);
    ...
    const infoLineParts = [];
    if (showOrderId && order.order_number)
        infoLineParts.push(`Order: ${order.order_number}`);
    if (showInvoiceNumber && order.invoice_number)
        infoLineParts.push(`Invoice: ${order.invoice_number}`);
    if (showOrderDate && order.order_date)
        infoLineParts.push(`Order Dt: ${order.order_date}`);
    if (showInvoiceDate && order.invoice_date)
        infoLineParts.push(`Inv Dt: ${order.invoice_date}`);
    if (showDeclaredValue)
        infoLineParts.push(`Declared: ${formatCurrency(order.order_amount)}`);
    ...
    if (includeProductName && chunk.length > 0) {
        ...
        const productRows = chunk.map((p) => {
            ...
            if (includeProductName)
                rowCells.push({ text: trimText(itemName, charLimit), fontSize: 7 });
            if (includeQty)
                rowCells.push({ text: String(qty), alignment: 'right', fontSize: 7 });
            if (includeCost)
                rowCells.push({ text: formatCurrency(price), alignment: 'right', fontSize: 7 });
            if (includeSku)
                rowCells.push({ text: trimText(sku, 14), fontSize: 7 });
            if (includeDimension)
                rowCells.push({ text: trimText(dim, 18), fontSize: 7 });
            if (includeDeadWeight)
                rowCells.push({ text: deadWeight, alignment: 'right', fontSize: 7 });
            return rowCells;
        });
        pageContent.push({
            table: {
    ...
    if (showOrderValueSection) {
        const productValue = products.reduce((sum, p) => {
        ...
        const codCollectible = paymentType === 'cod'
            ? codCollectibleRaw > 0
                ? codCollectibleRaw
                : orderValue
            : 0;
        const summaryParts = [`Order Value: ${formatCurrency(orderValue)}`];
        summaryParts.push(paymentType === 'cod'
    ...
    try {
        ...
        console.log(`📄 PDF generated successfully (${pdfBuffer.length} bytes) for order ${order?.order_number}`);
        // Upload
        ...
        const uploadResponse = await axios_1.default.put(finalUploadUrl, pdfBuffer, {
        ...
        console.error(`❌ Failed to generate/upload label for order ${order?.order_number}:`, err?.message || err, err?.stack);
