...
exports.developerIssueLabels = exports.retryFailedManifestServiceForAdmin = exports.updateDeveloperIssueStateService = exports.getDeveloperErrorLogsService = void 0;
...
const developerIssueAuditLogs_1 = require("../schema/developerIssueAuditLogs");
const developerIssueStates_1 = require("../schema/developerIssueStates");
const pendingWebhooks_1 = require("../schema/pendingWebhooks");
const userProfile_1 = require("../schema/userProfile");
const users_1 = require("../schema/users");
const webhookSubscriptions_1 = require("../schema/webhookSubscriptions");
...
const getMerchantName = (profile, user) => {
    return (profile?.companyInfo?.brandName ||
...
const getUserDisplayName = (user) => user?.email || user?.phone || null;
const buildForWhom = ({ merchantName, merchantEmail, buyerName, buyerPhone, orderNumber, awbNumber, affectedOrderCount, }) => {
...
const makeIssueKey = (source, parts) => `${source}:${parts.map((part) => normalizeKeyPart(part)).filter(Boolean).join(':')}`;
const getLatestByDate = (rows, selector) => {
    return rows
        .slice()
        .sort((a, b) => (selector(b)?.getTime() ?? 0) - (selector(a)?.getTime() ?? 0))[0];
};
const getEarliestDate = (dates) => dates
    .filter(Boolean)
    .sort((a, b) => a.getTime() - b.getTime())[0] ?? null;
const getLatestDate = (dates) => dates
    .filter(Boolean)
    .sort((a, b) => b.getTime() - a.getTime())[0] ?? null;
const getAgeHours = (date) => {
    if (!date)
        return 0;
    return (Date.now() - date.getTime()) / (1000 * 60 * 60);
};
const buildRelatedOrder = (order) => {
    ...
    return {
        ...
        can_retry_manifest: trimText(order.manifest_error).length > 0 &&
...
const classifyManifestIssue = (message, retryCount = 0) => {
    ...
    if (normalized.includes('non serviceable pincode')) {
        return {
            ...
            issueOwner: 'merchant',
            actionRequired: 'contact_merchant',
            priority: 'high',
            summary: 'Courier rejected the shipment because the destination or pickup pincode is not serviceable.',
            recommendation: 'Ask the merchant to change courier, address, or payment mode before trying again.',
...
const classifyPickupIssue = (message) => {
    const normalized = message.toLowerCase();
...
const classifyPendingWebhookIssue = (linkedOrder, rows) => {
...
const classifyWebhookDeliveryIssue = (latest, items) => {
    ...
    if (isServerSide) {
        return {
            ...
            issueOwner: 'merchant',
            actionRequired: exhausted ? 'contact_merchant' : 'ignore',
            priority: exhausted ? 'high' : 'low',
            summary: exhausted
    ...
    return {
        ...
        issueOwner: 'platform',
        actionRequired: exhausted ? 'escalate' : 'ignore',
        priority: exhausted ? 'high' : 'low',
        summary: exhausted
...
const createIssueAuditLog = async ({ issueKey, action, note, metadata, adminUserId, }) => {
    const normalizedIssueKey = trimText(issueKey);
    if (!normalizedIssueKey)
        return;
    await client_1.db.insert(developerIssueAuditLogs_1.developer_issue_audit_logs).values({
...
const syncIssueStates = async (issues) => {
    if (!issues.length)
        return new Map();
    const issueKeys = issues.map((issue) => issue.issue_key);
    const existingStates = await client_1.db
        .select()
        .from(developerIssueStates_1.developer_issue_states)
        .where((0, drizzle_orm_1.inArray)(developerIssueStates_1.developer_issue_states.issue_key, issueKeys));
    ...
    for (const issue of issues) {
        ...
        if (!existing) {
            const [inserted] = await client_1.db
                .insert(developerIssueStates_1.developer_issue_states)
                .values({
        ...
        const shouldAutoReopen = existing.status === 'resolved' &&
            issueLastSeen &&
            (!resolvedAt || issueLastSeen.getTime() > resolvedAt.getTime());
        const [updated] = await client_1.db
            .update(developerIssueStates_1.developer_issue_states)
            .set({
            source: issue.source,
            title: issue.title,
            last_seen_at: issueLastSeen,
            ...
            .where((0, drizzle_orm_1.eq)(developerIssueStates_1.developer_issue_states.issue_key, issue.issue_key))
            .returning();
        if (shouldAutoReopen) {
...
const buildGroupedIssues = async () => {
    const [orderErrors, pendingWebhookRows, failedWebhookDeliveries, subscriptions] = await Promise.all([
        client_1.db
            .select()
            .from(b2cOrders_1.b2c_orders)
            .where((0, drizzle_orm_1.sql) `(${b2cOrders_1.b2c_orders.manifest_error} is not null and trim(${b2cOrders_1.b2c_orders.manifest_error}) <> '') or (${b2cOrders_1.b2c_orders.pickup_error} is not null and trim(${b2cOrders_1.b2c_orders.pickup_error}) <> '')`)
            .orderBy((0, drizzle_orm_1.desc)(b2cOrders_1.b2c_orders.updated_at)),
        client_1.db
            .select()
            .from(pendingWebhooks_1.pending_webhooks)
            .where((0, drizzle_orm_1.sql) `${pendingWebhooks_1.pending_webhooks.processed_at} is null`)
            .orderBy((0, drizzle_orm_1.desc)(pendingWebhooks_1.pending_webhooks.created_at)),
        client_1.db
            .select()
            .from(webhookSubscriptions_1.webhook_deliveries)
            .where((0, drizzle_orm_1.eq)(webhookSubscriptions_1.webhook_deliveries.status, 'failed'))
            .orderBy((0, drizzle_orm_1.desc)(webhookSubscriptions_1.webhook_deliveries.created_at)),
        client_1.db.select().from(webhookSubscriptions_1.webhook_subscriptions),
    ]);
    const pendingWebhookAwbs = Array.from(new Set(pendingWebhookRows
        .map((row) => trimText(row.awb_number))
        .filter((awb) => Boolean(awb))));
    const pendingWebhookOrders = pendingWebhookAwbs.length > 0
        ? await client_1.db.select().from(b2cOrders_1.b2c_orders).where((0, drizzle_orm_1.inArray)(b2cOrders_1.b2c_orders.awb_number, pendingWebhookAwbs))
        : [];
    const subscriptionMap = new Map(subscriptions.map((row) => [row.id, row]));
    const pendingOrderMap = new Map(pendingWebhookOrders.map((row) => [trimText(row.awb_number), row]));
    ...
    const [profiles, userRows] = userIds.length > 0
        ? await Promise.all([
            client_1.db.select().from(userProfile_1.userProfiles).where((0, drizzle_orm_1.inArray)(userProfile_1.userProfiles.userId, userIds)),
            client_1.db.select().from(users_1.users).where((0, drizzle_orm_1.inArray)(users_1.users.id, userIds)),
        ])
        : [[], []];
    ...
    for (const [issueKey, rows] of pendingGroups.entries()) {
        ...
        issues.push({
            ...
            recommendation: classification.recommendation,
            why_this_matters: classification.whyThisMatters,
            raw_error: clampText(linkedOrder
    ...
    for (const delivery of failedWebhookDeliveries) {
        ...
        if (!webhookGroups.has(key))
            webhookGroups.set(key, []);
    ...
    for (const [issueKey, rows] of webhookGroups.entries()) {
        ...
        issues.push({
            ...
            how_it_happened: `Platform tried to send an outbound webhook. Latest failure was attempt ${Number(latest.attempt_count ?? 0)}/${Number(latest.max_attempts ?? 0)} with ${latest.http_status ? `HTTP ${latest.http_status}` : 'no HTTP response'}.`,
            for_whom: buildForWhom({
                merchantName,
                merchantEmail,
            ...
            recommendation: classification.recommendation,
            why_this_matters: classification.whyThisMatters,
            raw_error: clampText(trimText(latest.error_message) || trimText(latest.response_body) || 'Webhook delivery failed.'),
            raw_payload: stringifyJson({
            ...
            occurrence_count: rows.length,
            affected_order_count: Array.from(new Set(rows.map((row) => trimText(row.payload?.order_number)).filter(Boolean))).length,
            can_retry_manifest: false,
            retryable_order_count: 0,
            related_orders: [],
            related_pending_webhooks: [],
...
const filterRows = (rows, filters) => {
    ...
    if (courierSearch) {
        nextRows = nextRows.filter((row) => String(row.courier_partner || '').toLowerCase().includes(courierSearch));
    }
    const merchantSearch = trimText(filters.merchant).toLowerCase();
    if (merchantSearch) {
        nextRows = nextRows.filter((row) => [row.merchant_name, row.merchant_email]
            .filter(Boolean)
...
const getDeveloperErrorLogsService = async ({ page = 1, limit = 20, filters = {}, }) => {
    ...
    const adminIds = Array.from(new Set(issueStates
        .flatMap((state) => [state.owner_admin_id, state.resolved_by_admin_id])
        .filter((id) => Boolean(id))));
    const adminUsers = adminIds.length > 0 ? await client_1.db.select().from(users_1.users).where((0, drizzle_orm_1.inArray)(users_1.users.id, adminIds)) : [];
    const adminUserMap = new Map(adminUsers.map((admin) => [admin.id, admin]));
    let rows = activeIssues.map((issue) => {
    ...
    rows.sort((a, b) => {
        if (a.has_new_alert !== b.has_new_alert)
            return a.has_new_alert ? -1 : 1;
        if (a.status_label !== b.status_label)
            return a.status_label === 'open' ? -1 : 1;
    ...
    const auditRows = paginatedIssueKeys.length > 0
        ? await client_1.db
            .select()
            .from(developerIssueAuditLogs_1.developer_issue_audit_logs)
            .where((0, drizzle_orm_1.inArray)(developerIssueAuditLogs_1.developer_issue_audit_logs.issue_key, paginatedIssueKeys))
            .orderBy((0, drizzle_orm_1.desc)(developerIssueAuditLogs_1.developer_issue_audit_logs.created_at))
        : [];
    const auditAdminIds = Array.from(new Set(auditRows.map((row) => row.admin_user_id).filter((id) => Boolean(id))));
    if (auditAdminIds.length > 0) {
        const missingAdminIds = auditAdminIds.filter((id) => !adminUserMap.has(id));
        if (missingAdminIds.length > 0) {
            const moreAdmins = await client_1.db.select().from(users_1.users).where((0, drizzle_orm_1.inArray)(users_1.users.id, missingAdminIds));
            moreAdmins.forEach((admin) => adminUserMap.set(admin.id, admin));
        }
    }
    const auditTrailMap = new Map();
    for (const audit of auditRows) {
        const admin = audit.admin_user_id ? adminUserMap.get(audit.admin_user_id) : null;
        if (!auditTrailMap.has(audit.issue_key))
            auditTrailMap.set(audit.issue_key, []);
    ...
    const filterMeta = {
        ...
        rootCauses: Array.from(new Map(rows.map((row) => [row.root_cause_key, { value: row.root_cause_key, label: row.root_cause_label }])).values()),
    };
    return {
        logs,
        totalCount,
...
exports.getDeveloperErrorLogsService = getDeveloperErrorLogsService;
const updateDeveloperIssueStateService = async ({ issueKey, adminUserId, status, priority, assignToMe, clearOwner, markAlertSeen, }) => {
    const normalizedIssueKey = trimText(issueKey);
    if (!normalizedIssueKey) {
        throw new classes_1.HttpError(400, 'Issue key is required.');
    }
    const [issueState] = await client_1.db
        .select()
        .from(developerIssueStates_1.developer_issue_states)
        .where((0, drizzle_orm_1.eq)(developerIssueStates_1.developer_issue_states.issue_key, normalizedIssueKey))
        .limit(1);
    if (!issueState) {
        throw new classes_1.HttpError(404, 'Issue not found.');
    }
    const nextStatus = status && ['open', 'resolved'].includes(status) ? status : undefined;
    const nextPriority = priority && ['high', 'medium', 'low'].includes(priority)
        ? priority
        : undefined;
    const updateData = {
        updated_at: new Date(),
    ...
    const [updated] = await client_1.db
        ...
        .where((0, drizzle_orm_1.eq)(developerIssueStates_1.developer_issue_states.issue_key, normalizedIssueKey))
        .returning();
    await Promise.all(auditEntries.map((entry) => createIssueAuditLog({
...
exports.updateDeveloperIssueStateService = updateDeveloperIssueStateService;
const retryFailedManifestServiceForAdmin = async ({ orderId, issueKey, adminUserId, }) => {
    ...
    const [order] = await client_1.db
        ...
        .from(b2cOrders_1.b2c_orders)
        .where((0, drizzle_orm_1.eq)(b2cOrders_1.b2c_orders.id, normalizedOrderId))
        .limit(1);
    if (!order) {
        throw new classes_1.HttpError(404, 'Order not found.');
    }
    if (String(order.order_status || '').toLowerCase() !== 'manifest_failed') {
        throw new classes_1.HttpError(400, 'Only manifest_failed orders can be retried from Developer tab.');
    }
    try {
        const result = await (0, shiprocket_service_1.retryFailedManifestService)(order.id, order.user_id);
        if (issueKey) {
            await createIssueAuditLog({
                issueKey,
...
exports.retryFailedManifestServiceForAdmin = retryFailedManifestServiceForAdmin;
exports.developerIssueLabels = {
    ISSUE_OWNER_LABELS,
