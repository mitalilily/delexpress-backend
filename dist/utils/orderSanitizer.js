...
exports.sanitizeOrdersForCustomer = exports.sanitizeOrderForCustomer = void 0;
 ...
 */
const ensureDownloadUrl = async (value) => {
    ...
    try {
        const url = await (0, upload_service_1.presignDownload)(value);
 ...
 */
const sanitizeOrderForCustomer = async (order) => {
    if (!order)
        return order;
    ...
    if (order.label)
        sanitized.label_key = order.label;
    if (order.manifest)
        sanitized.manifest_key = order.manifest;
    if (order.invoice_link)
        sanitized.invoice_key = order.invoice_link;
    try {
...
exports.sanitizeOrderForCustomer = sanitizeOrderForCustomer;
const sanitizeOrdersForCustomer = async (orders) => {
    return Promise.all(orders.map(exports.sanitizeOrderForCustomer));
};
exports.sanitizeOrdersForCustomer = sanitizeOrdersForCustomer;
