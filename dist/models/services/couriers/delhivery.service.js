...
exports.DelhiveryService = void 0;
...
class DelhiveryService {
    constructor() {
        this.apiBase = 'https://track.delhivery.com';
        this.token = '';
        this.clientName = '';
    ...
    async postFormEncoded(path, payload) {
        await this.ensureCredentials();
        const encodedData = qs_1.default.stringify({
            format: 'json',
            data: JSON.stringify(payload),
        });
        return axios_1.default.post(`${this.apiBase}${path}`, encodedData, {
            headers: {
                Authorization: `Token ${this.token}`,
                Accept: 'application/json',
    ...
    async getExpectedTAT(origin, destination, mot = 'S', pdt = 'B2C') {
        try {
            await this.ensureCredentials();
    ...
    async createShipment(params, waybill) {
        try {
            const normalizedCourierId = (0, delhiveryCourier_1.normalizeCourierId)(params.courier_id);
            if (normalizedCourierId === null) {
                throw new classes_1.HttpError(400, 'Delhivery courier_id is required for Surface/Express bookings. Provide 99 for Surface or 100 for Express.');
            }
            const shippingMode = (0, delhiveryCourier_1.getDelhiveryShippingModeByCourierId)(normalizedCourierId);
            if (!shippingMode) {
                throw new classes_1.HttpError(400, `Invalid Delhivery courier_id: ${normalizedCourierId}. Allowed IDs are 100 (Express) and 99 (Surface).`);
            }
            ...
            const hsnCodes = Array.from(new Set(orderItems
                .map((item) => (item?.hsn || item?.hsnCode || '').toString().trim())
                .filter((code) => code.length > 0)));
            if (!orderNumber) {
                throw new classes_1.HttpError(400, 'order_number is required to create a Delhivery shipment.');
            }
            if (!invoiceNumber) {
                console.warn(`ℹ️ Delhivery invoice_number missing for order ${orderNumber}; using order_number as fallback.`);
            }
            // if (!invoiceNumber) {
            //   throw new HttpError(
            //     400,
            ...
            if (orderAmount <= 0 || Number.isNaN(orderAmount)) {
                throw new classes_1.HttpError(400, 'order_amount is required and must be a positive number when booking with Delhivery.');
            }
            if ((params.mps || boxes.length > 1) && !waybill) {
                throw new classes_1.HttpError(400, 'Delhivery multi-piece shipment is not supported in the current B2C flow. Use a single-package shipment.');
            }
            const pickupAddressParts = [
                sanitizeString(pickup.address),
                sanitizeString(pickup.address_2),
            ].filter((part) => part.length > 0);
            const pickupAddress = pickupAddressParts.length > 0
            ...
            if (!consigneePhone) {
                throw new classes_1.HttpError(400, 'Consignee phone must contain at least 10 digits for Delhivery shipments.');
            }
            const pickupPhone = sanitizePhone(pickup.phone);
            if (!pickupPhone) {
                throw new classes_1.HttpError(400, 'Valid pickup phone is required for Delhivery manifests.');
            }
            const orderDate = params.order_date instanceof Date
                ? params.order_date.toISOString().split('T')[0]
                : sanitizeString(params.order_date) || new Date().toISOString().split('T')[0];
            const invoiceDate = params.invoice_date && sanitizeString(params.invoice_date)
                ? sanitizeString(params.invoice_date)
                : orderDate;
            const paymentMode = params.payment_type === 'cod'
                ? 'COD'
            ...
            const ewbnValue = params.ewbn || params.ewb || params.ewbn_number || params.ewaybill_number || undefined;
            if (ewbnValue) {
            ...
            const resolvedReturnAddress = params.rto && params.is_rto_different === 'yes'
                ? params.rto
                : paymentMode === 'REPL'
            ...
            const packages = Array.isArray(responseData?.packages) && responseData.packages.length
            ...
            const packageFailures = packages.filter((pkg) => normalizedStatus(pkg?.status) === 'fail' || pkg?.serviceable === false || !pkg?.waybill);
            const packageFailuresWithRemarks = packageFailures.map((pkg) => ({
                ...pkg,
                remarks: normalizeRemarks(pkg?.remarks),
            }));
            const successPackage = packages.find((pkg) => pkg?.waybill && pkg?.serviceable !== false && normalizedStatus(pkg?.status) !== 'fail');
            if (overallStatus === 'fail' ||
                ...
                throw new classes_1.DelhiveryManifestError(502, failureReason, responseData);
            }
            const responseShippingMode = responseData?.shipping_mode ??
            ...
            if (awb) {
                ...
                while (retries > 0 && !labelMetaFetched) {
                    try {
                        ...
                        console.warn(`⚠️ Failed to fetch Delhivery label metadata (attempt ${4 - retries}/3):`, err?.message || err);
                        if (retries > 1) {
                ...
                if (!labelMetaFetched) {
                    console.error(`❌ Failed to fetch Delhivery label metadata after all retries for AWB: ${awb}`);
                }
            }
    ...
    async cancelShipment(waybill) {
        try {
            ...
            const res = await axios_1.default.post(`${this.apiBase}/api/p/edit`, { waybill, cancellation: 'true' }, {
    ...
    async trackShipment(awb) {
        await this.ensureCredentials();
        const res = await axios_1.default.get(`${this.apiBase}/api/v1/packages/json/?waybill=${awb}`, {
            headers: this.headers,
        });
        return res.data;
    }
    // 🔹 8. NDR Action (RE-ATTEMPT / PICKUP_RESCHEDULE)
    async submitNdrAction(actions) {
        try {
            await this.ensureCredentials();
            ...
            const payload = actions.map((action) => {
                ...
                if (mappedAct === 'DEFER_DLV') {
                    const normalizedDeferredDate = actionData.deferred_date || actionData.deferment_date || actionData.defermentDate;
            ...
            const res = await axios_1.default.post(url, { data: payload }, { headers: this.headers });
    ...
    async getNdrStatus(uplId, verbose = true) {
        try {
            await this.ensureCredentials();
            const url = `${this.apiBase}/api/cmu/get_bulk_upl/${encodeURIComponent(uplId)}?verbose=${verbose ? 'true' : 'false'}`;
            const res = await axios_1.default.get(url, { headers: this.headers });
            return res.data;
        }
        catch (err) {
    ...
    async requestPickup(pickupData) {
        await this.ensureCredentials();
        const res = await axios_1.default.post(`${this.apiBase}/fm/request/new/`, pickupData, {
            headers: this.headers,
        });
        return res.data;
    }
    ...
    async createWarehouse(warehouse) {
        try {
            ...
            const res = await axios_1.default.post(url, warehouse, { headers });
            return res.data;
        }
        catch (err) {
    ...
    async triggerDelhiveryPickupRequest(pickupLocationName, packageCount) {
        try {
    ...
    async createReverseShipment(params) {
        try {
                ...
                shipments: [
                    {
                        ...
                        cod_amount: '0',
                        products_desc: params.order_items?.map((i) => i.name).join(', ') || 'Reverse Pickup Shipment',
                        weight: Number(params.package_weight ?? 0.5),
                        shipment_length: Number(params.package_length ?? 10),
                        shipment_width: Number(params.package_breadth ?? 10),
                        shipment_height: Number(params.package_height ?? 10),
            ...
            const delhiveryCost = pkg?.charge || pkg?.amount || res.data?.charge || res.data?.amount || null;
            return {
                success: true,
    ...
    async createPickupRequest({ pickup_date, pickup_time, pickup_location, expected_package_count, }) {
        try {
            await this.ensureCredentials();
            ...
            const providerMessage = typeof providerError?.pickup_date === 'string'
            ...
            error.details = providerError || null;
            error.isPickupRequestError = true;
            throw error;
        }
    }
    ...
    async generateLabel(awb, options = { format: 'json' }) {
        await this.ensureCredentials();
        const format = options.format || 'json';
        const url = `${this.apiBase}/api/p/packing_slip?wbns=${encodeURIComponent(awb)}${format === 'pdf' ? '&pdf=true' : '&pdf=false'}`;
...
exports.DelhiveryService = DelhiveryService;
