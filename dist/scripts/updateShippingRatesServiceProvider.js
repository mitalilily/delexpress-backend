...
async function updateShippingRatesServiceProvider() {
    try {
        ...
        const result = await client_1.pool.query('SELECT id, courier_id, courier_name, service_provider FROM shipping_rates');
        ...
        for (const rate of ratesArray) {
            try {
                ...
                const matchingCouriers = await client_1.db
                    ...
                    .where((0, drizzle_orm_1.eq)(couriers_1.couriers.id, rate.courier_id));
                if (matchingCouriers.length === 0) {
                    console.warn(`⚠️  No courier found for courier_id: ${rate.courier_id} (shipping_rate id: ${rate.id})`);
                    skippedCount++;
                    errors.push({
                ...
                if (!matchedCourier) {
                    ...
                    console.warn(`⚠️  Courier name mismatch for courier_id ${rate.courier_id}: shipping_rate has "${rate.courier_name}", courier has "${matchedCourier.name}". Using first match.`);
                }
                ...
                await client_1.pool.query('UPDATE shipping_rates SET service_provider = $1 WHERE id = $2', [
                    correctServiceProvider,
                    rate.id,
                ]);
                updatedCount++;
                if (updatedCount <= 10 || updatedCount % 100 === 0) {
                    console.log(`✅ Corrected shipping_rate ${rate.id}: courier_id=${rate.courier_id}, courier_name="${rate.courier_name}" → service_provider changed from "${currentServiceProvider || 'NULL'}" to "${correctServiceProvider}"`);
                }
                ...
                console.error(`❌ Error updating shipping_rate ${rate.id} (courier_id: ${rate.courier_id}):`, error);
                skippedCount++;
                errors.push({
                    courier_id: rate.courier_id,
                    courier_name: rate.courier_name,
        ...
        if (errors.length > 0) {
            ...
            errors.slice(0, 10).forEach((err) => {
                console.log(`  - courier_id ${err.courier_id}, courier_name "${err.courier_name}": ${err.error}`);
            });
            if (errors.length > 10) {
