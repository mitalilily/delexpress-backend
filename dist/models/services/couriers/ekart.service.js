...
exports.EkartService = void 0;
const axios_1 = __importDefault(require("axios"));
const courierCredentials_service_1 = require("../courierCredentials.service");
const classes_1 = require("../../../utils/classes");
class EkartService {
    constructor() {
        this.baseApi = process.env.EKART_BASE_API || courierCredentials_service_1.DEFAULT_EKART_BASE_URL;
        this.baseAuth = process.env.EKART_BASE_AUTH || courierCredentials_service_1.DEFAULT_EKART_BASE_URL;
    ...
    normalizeBaseUrl(value) {
        return (0, courierCredentials_service_1.normalizeEkartBaseUrl)(value) || courierCredentials_service_1.DEFAULT_EKART_BASE_URL;
    }
    maskPhone(value) {
    ...
    sanitizeShipmentPayload(payload) {
        return {
            ...
            consignee_phone: this.maskPhone(payload?.consignee_phone ?? payload?.consignee_alternate_phone ?? payload?.drop?.phone),
    ...
    getNormalizedInvoiceDate(payload) {
    ...
    sanitizePhoneNumber(value, fallback = '') {
        const digits = String(value ?? '').replace(/\D/g, '');
        if (!digits)
            return fallback;
        return digits.length <= 10 ? digits : digits.slice(-10);
    }
    normalizePin(value, fallback = 0) {
        const digits = String(value ?? '').replace(/\D/g, '');
        if (!digits)
            return fallback;
        return Number(digits.slice(0, 6));
    }
    buildEkartShipmentPayload(payload) {
        const paymentMode = String(payload?.payment_type || '').toLowerCase() === 'cod'
            ? 'COD'
        ...
        const taxableAmount = normalizedItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
        const totalTaxValue = Number(normalizedItems.reduce((sum, item) => sum + this.toNumber(item.tax_value, 0), 0).toFixed(2));
        ...
        const categoryOfGoods = this.sanitizeText(payload?.category_of_goods || normalizedItems.map((item) => item.name).join(', '), 'General Merchandise');
        return {
            ...
            products_desc: normalizedItems.map((item) => item.name).join(', '),
            ...
            pickup_location: {
                name: this.sanitizeText(payload?.pickup_location_alias || payload?.pickup?.warehouse_name || payload?.pickup?.name, pickupContact.name),
            },
            drop_location: {
            ...
            return_location: {
                name: this.sanitizeText(payload?.return_location_alias || payload?.rto?.warehouse_name || returnContact.name, returnContact.name),
            },
            package: {
    ...
    isLocationNotRegisteredError(err) {
        ...
        const errorText = [
            ...
            .join(' ')
            .toLowerCase();
        return (errorText.includes('swift_resource_not_found_exception') &&
            errorText.includes('location') &&
            errorText.includes('not registered'));
    }
    buildWarehousePayloadFromShipmentPayload(payload) {
        const pickup = payload?.pickup || {};
        const alias = this.sanitizeText(payload?.pickup_location?.name || pickup?.warehouse_name || pickup?.name);
        ...
        return {
            ...
            address_line1: this.sanitizeText(pickup?.address),
            address_line2: this.sanitizeText(pickup?.address_2) || null,
    ...
    async registerPickupLocationFromShipmentPayload(payload) {
        const warehousePayload = this.buildWarehousePayloadFromShipmentPayload(payload);
        await this.createWarehouse(warehousePayload);
        return warehousePayload.alias;
    }
    async ensureConfigLoaded() {
        if (EkartService.cachedConfig === undefined) {
            EkartService.cachedConfig = await (0, courierCredentials_service_1.getEffectiveCourierConfig)('ekart', 'b2c');
        }
    ...
    async getAccessToken() {
        ...
        let res;
        try {
            res = await axios_1.default.post(url, { username: this.username, password: this.password });
        }
        catch (err) {
            ...
            throw new classes_1.HttpError(Number(err?.response?.status || 502), this.extractErrorMessage(err, 'Ekart authentication failed'));
        }
    ...
    async checkPincodeServiceability(pincode) {
        ...
        for (const candidate of candidates) {
            try {
                ...
                if (![404, 405].includes(status)) {
                    throw new classes_1.HttpError(Number(err?.response?.status || 502), this.extractErrorMessage(err, 'Ekart pincode serviceability failed'));
                }
            }
        }
        ...
        throw new classes_1.HttpError(502, 'Ekart pincode serviceability failed');
    }
    async checkPairServiceability(payload) {
        const http = await this.getHttp();
        const normalizedPaymentType = String(payload.paymentType || 'Prepaid').trim().toLowerCase() === 'cod'
            ? 'cod'
            : 'prepaid';
        ...
        const candidates = [
            {
                ...
                execute: () => http.get('/data/global/serviceability', {
                    params: {
        ...
        for (const candidate of candidates) {
            try {
                ...
                if (![404, 405].includes(status)) {
                    ...
                    throw new classes_1.HttpError(Number(err?.response?.status || 502), this.extractErrorMessage(err, 'Ekart serviceability failed'));
                }
            }
        }
        if (!res || !succeededWith) {
            ...
            throw new classes_1.HttpError(Number(lastFailure?.status || 502), 'Ekart serviceability failed');
        }
                    ...
                    : Array.isArray(raw?.serviceability)
                        ...
                        : Array.isArray(raw?.services)
                            ...
                            : raw && typeof raw === 'object'
                                ? [raw]
                                : [];
        const availability = records.find((record) => record && typeof record === 'object') ??
        ...
        const hasTatSignal = (typeof availability?.tat === 'number' && availability.tat > 0) ||
            (availability?.tat &&
        ...
        const hasChargeSignal = Boolean(availability?.forwardDeliveredCharges ||
        ...
        const serviceable = toBoolean(availability?.is_serviceable, availability?.serviceable, availability?.available, availability?.status, raw?.is_serviceable, raw?.serviceable, raw?.available, raw?.status) ??
            (records.length > 0 && (hasTatSignal || hasChargeSignal));
        const codAvailable = toBoolean(availability?.cod, availability?.is_cod, availability?.cod_available, raw?.cod, raw?.is_cod, raw?.cod_available) ?? true;
        const prepaidAvailable = toBoolean(availability?.prepaid, availability?.prepaid_available, availability?.is_prepaid, raw?.prepaid, raw?.prepaid_available, raw?.is_prepaid) ?? true;
        const tatCandidates = [
            availability?.tat,
    ...
    async createShipment(payload) {
        ...
        try {
            ...
            if (this.isLocationNotRegisteredError(err)) {
                try {
                    const alias = await this.registerPickupLocationFromShipmentPayload(normalizedPayload);
                    ...
                    throw new classes_1.HttpError(Number(retryErr?.response?.status || err?.response?.status || 502), this.extractErrorMessage(retryErr, this.extractErrorMessage(err, 'Ekart shipment creation failed')));
                }
            }
            ...
            throw new classes_1.HttpError(Number(err?.response?.status || 502), this.extractErrorMessage(err, 'Ekart shipment creation failed'));
        }
    }
    ...
    // ---------- Labels & Manifest ----------
    async downloadLabels(ids, jsonOnly = false) {
        const http = await this.getHttp();
        const res = await http.post('/api/v1/package/label', { ids }, {
            params: { json_only: jsonOnly },
            responseType: jsonOnly ? 'json' : 'arraybuffer',
        });
        return res.data;
    }
    async generateManifest(ids) {
        const http = await this.getHttp();
    ...
    async submitNdrAction(payload) {
        return this.ndrAction(payload);
    }
}
exports.EkartService = EkartService;
