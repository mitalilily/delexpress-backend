...
exports.seedHolidaysCron = void 0;
 ...
 */
const seedHolidaysCron = async ({ force = false } = {}) => {
    ...
    if (!force && (currentMonth !== 1 || currentDay !== 1)) {
        console.log(`⏭️ Skipping holiday seeding: not January 1st (today: ${currentDay}/${currentMonth}/${currentYear})`);
        return;
    }
    ...
    try {
        // Seed current year
        console.log(`\n📅 Seeding ${currentYear}...`);
        const currentResult = await (0, holiday_service_1.seedDefaultNationalHolidays)(currentYear);
        console.log(`   ✅ ${currentYear}: Created ${currentResult.created.length}, Skipped ${currentResult.skipped.length}`);
        // Seed next year
        const nextYear = currentYear + 1;
        console.log(`\n📅 Seeding ${nextYear}...`);
        const nextResult = await (0, holiday_service_1.seedDefaultNationalHolidays)(nextYear);
        console.log(`   ✅ ${nextYear}: Created ${nextResult.created.length}, Skipped ${nextResult.skipped.length}`);
...
exports.seedHolidaysCron = seedHolidaysCron;
