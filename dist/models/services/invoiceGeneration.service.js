...
exports.regenerateInvoiceWithAdjustments = exports.generateInvoiceForUser = void 0;
...
const invoicePreferences_service_1 = require("./invoicePreferences.service");
...
const generateInvoiceForUser = async (userId, { startDate, endDate }) => {
    console.log(`🧾 Generating invoice for ${userId} (${(0, dayjs_1.default)(startDate).format('DD MMM')} → ${(0, dayjs_1.default)(endDate).format('DD MMM')})`);
    ...
    const b2cOrders = await client_1.db
        ...
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(b2cOrders_1.b2c_orders.user_id, userId), (0, drizzle_orm_1.between)(b2cOrders_1.b2c_orders.created_at, startDate, endDate), (0, drizzle_orm_1.inArray)(b2cOrders_1.b2c_orders.order_status, billableStatuses)));
    const b2bOrders = await client_1.db
        .select()
        .from(b2bOrders_1.b2b_orders)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(b2bOrders_1.b2b_orders.user_id, userId), (0, drizzle_orm_1.between)(b2bOrders_1.b2b_orders.created_at, startDate, endDate), (0, drizzle_orm_1.inArray)(b2bOrders_1.b2b_orders.order_status, billableStatuses)));
    const allOrders = [...b2cOrders, ...b2bOrders];
    if (allOrders.length === 0) {
        console.log(`⚠️ No billable orders for user ${userId} between ${(0, dayjs_1.default)(startDate).format('YYYY-MM-DD HH:mm:ss')} and ${(0, dayjs_1.default)(endDate).format('YYYY-MM-DD HH:mm:ss')} (statuses: ${billableStatuses.join(', ')})`);
        return null;
    }
    // 2️⃣ Totals
    let totalShipping = 0;
    ...
    for (const o of allOrders) {
        ...
        const cb = anyOrder.charges_breakdown;
        if (cb && typeof cb === 'object') {
            ...
            if (Array.isArray(cb.overheads)) {
                for (const oh of cb.overheads) {
                    if (!oh || typeof oh.amount !== 'number' || oh.amount === 0)
                        continue;
    ...
    const prefs = await (0, invoicePreferences_service_1.getInvoicePreferences)(userId);
    const adminPrefs = await (0, invoicePreferences_service_1.getAdminInvoicePreferences)();
    const adminLogoFile = adminPrefs?.logoFile ?? null;
    let adminSignatureBuffer;
    ...
    try {
        if (adminIncludeSignature && adminPrefs?.signatureFile) {
            ...
            const signatureUrl = await (0, upload_service_1.presignDownload)(adminPrefs.signatureFile);
            if (signatureUrl) {
        ...
        console.warn(`⚠️ [Billing Invoice] Failed to load admin signature:`, err?.message || err, err?.code === 'ECONNABORTED' ? '(timeout)' : '');
        // Continue without admin signature - will show text fallback
    }
    const rawInvoiceId = `${(0, dayjs_1.default)().format('YYMMDD')}${invoiceId.slice(0, 6).toUpperCase()}`;
    const invoiceNo = `${prefs?.prefix ?? 'INV'}${rawInvoiceId}${prefs?.suffix ?? ''}`;
    const invoiceDate = (0, dayjs_1.default)().format('DD MMM YYYY');
    const invoicePeriod = `${(0, dayjs_1.default)(startDate).format('DD MMM YYYY')} - ${(0, dayjs_1.default)(endDate).format('DD MMM YYYY')}`;
    const tmpDir = path_1.default.join(process.cwd(), 'tmp', 'invoices');
    if (!fs_1.default.existsSync(tmpDir))
        fs_1.default.mkdirSync(tmpDir, { recursive: true });
    const pdfPath = path_1.default.join(tmpDir, `${invoiceNo}.pdf`);
    const csvPath = path_1.default.join(tmpDir, `${invoiceNo}.csv`);
    // 4️⃣ Generate CSV
    ...
    const csvBody = allOrders
        ...
        const orderType = o.order_type || 'prepaid';
        const orderTypeDisplay = orderType.toLowerCase() === 'cod' ? 'COD' : 'Prepaid';
        const billedDate = o.updated_at || o.created_at;
        return `${o.order_id || o.order_number},${orderTypeDisplay},${o.awb_number || '-'},${o.freight_charges ?? o.shipping_charges ?? 0},${o.cod_charges ?? 0},${(0, dayjs_1.default)(billedDate).format('YYYY-MM-DD')}`;
    })
        .join('\n');
    fs_1.default.writeFileSync(csvPath, csvHeader + csvBody, 'utf-8');
    // 5️⃣ Get seller info
    const [sellerRow] = await client_1.db
        .select()
    ...
    const bufferToDataUrl = async (buffer) => {
        try {
            ...
            if (!type) {
                ...
                (buffer[0] === 0x89 && buffer[1] === 0x50 && buffer[2] === 0x4e && buffer[3] === 0x47) ||
                    ...
                    (buffer.length >= 12 &&
                        ...
                        buffer[11] === 0x50);
                if (!isValidImage) {
                    console.warn('⚠️ Could not detect image type and buffer does not appear to be a valid image format');
                    return null;
                }
                console.warn('⚠️ Could not detect image type via file-type, but buffer appears valid, defaulting to PNG');
    ...
    try {
        if (prefs?.includeLogo && prefs?.logoFile) {
            const logoUrl = await (0, upload_service_1.presignDownload)(prefs.logoFile);
            const finalUrl = Array.isArray(logoUrl) ? logoUrl[0] : logoUrl;
            if (finalUrl) {
                const logoResponse = await axios_1.default.get(finalUrl, { responseType: 'arraybuffer' });
    ...
    if (adminSignatureBuffer) {
        try {
            ...
            console.warn('⚠️ [Billing Invoice] Failed to convert admin signature buffer to data URL:', err);
        }
    }
    // Platform (DelExpress) logo for footer branding
    let platformLogoDataUrl;
    try {
        const platformLogoKey = adminLogoFile || 'logo-white.png';
        const logoUrl = await (0, upload_service_1.presignDownload)(platformLogoKey);
        if (logoUrl && typeof logoUrl === 'string') {
            const resp = await axios_1.default.get(logoUrl, { responseType: 'arraybuffer' });
    ...
    if (logoDataUrl && typeof logoDataUrl === 'string' && logoDataUrl.startsWith('data:image/')) {
        ...
        console.warn('⚠️ [Billing Invoice] Logo data URL is not in valid format:', typeof logoDataUrl, logoDataUrl?.substring(0, 50));
    }
    ...
    if (adminSignatureDataUrl &&
        ...
        console.log('ℹ️ [Billing Invoice] Admin signature preference is enabled but signature file not available');
    }
    else if (adminSignatureDataUrl) {
        console.warn('⚠️ [Billing Invoice] Admin signature data URL is not in valid format:', typeof adminSignatureDataUrl, adminSignatureDataUrl?.substring(0, 50));
    }
    ...
    if (platformLogoDataUrl &&
        ...
        console.warn('⚠️ [Billing Invoice] Platform logo data URL is not in valid format:', typeof platformLogoDataUrl, platformLogoDataUrl?.substring(0, 50));
    }
    const docDefinition = {
    ...
    // Upload to R2
    const pdfUpload = await (0, upload_service_1.presignUpload)({
        filename: `${invoiceNo}.pdf`,
        contentType: 'application/pdf',
        userId,
        folderKey: 'invoices',
    });
    const csvUpload = await (0, upload_service_1.presignUpload)({
    ...
    const csvBuffer = fs_1.default.readFileSync(csvPath);
    await Promise.all([
    ...
    const [invoice] = await client_1.db
        .insert(billingInvoices_1.billingInvoices)
        .values({
        invoiceNo,
        sellerId: userId,
    ...
    let pdfSignedUrl = undefined;
    let csvSignedUrl = undefined;
    try {
        const signedUrls = await (0, upload_service_1.presignDownload)([pdfKey, csvKey]);
        ...
        console.error(`Failed to presign URLs for invoice ${invoiceNo} (email will be sent without download links):`, presignErr?.message || presignErr);
    ...
    if (sellerEmail) {
        try {
            await (0, emailSender_1.sendInvoiceReadyEmail)({
                to: sellerEmail,
                sellerName: billTo,
                invoiceNo,
                periodStart: (0, dayjs_1.default)(startDate).format('DD MMM YYYY'),
                periodEnd: (0, dayjs_1.default)(endDate).format('DD MMM YYYY'),
                totalAmount,
                pdfUrl: pdfSignedUrl,
                csvUrl: csvSignedUrl,
                attachFiles: false,
    ...
    console.log(`✅ Invoice generated: ${invoiceNo} → ${formatAmount(totalAmount)} (${allOrders.length} orders)`);
    // Auto-mark as paid if outstanding is already 0 (e.g., already paid via wallet at order time)
    try {
        const { getInvoiceStatement } = await Promise.resolve().then(() => __importStar(require('./invoiceStatement.service')));
...
exports.generateInvoiceForUser = generateInvoiceForUser;
// Regenerate invoice PDF/CSV with adjustments included
const regenerateInvoiceWithAdjustments = async (invoiceId) => {
    ...
    const orderNumbers = inv.orderNumbers || [];
    let allOrders = [];
    if (orderNumbers.length > 0) {
        const [b2c, b2b] = await Promise.all([
            client_1.db
                .select()
                .from(b2cOrders_1.b2c_orders)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(b2cOrders_1.b2c_orders.user_id, inv.sellerId), (0, drizzle_orm_1.inArray)(b2cOrders_1.b2c_orders.order_number, orderNumbers))),
            client_1.db
                .select()
                .from(b2bOrders_1.b2b_orders)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(b2bOrders_1.b2b_orders.user_id, inv.sellerId), (0, drizzle_orm_1.inArray)(b2bOrders_1.b2b_orders.order_number, orderNumbers))),
        ]);
        allOrders = [...(b2c || []), ...(b2b || [])];
    }
    else {
        // Fallback to date range
        const startDate = new Date(inv.billingStart);
        const endDate = new Date(inv.billingEnd);
        const billableStatuses = [...BILLABLE_ORDER_STATUSES];
        const [b2c, b2b] = await Promise.all([
            client_1.db
                .select()
                .from(b2cOrders_1.b2c_orders)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(b2cOrders_1.b2c_orders.user_id, inv.sellerId), (0, drizzle_orm_1.between)(b2cOrders_1.b2c_orders.created_at, startDate, endDate), (0, drizzle_orm_1.inArray)(b2cOrders_1.b2c_orders.order_status, billableStatuses))),
            client_1.db
                .select()
                .from(b2bOrders_1.b2b_orders)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(b2bOrders_1.b2b_orders.user_id, inv.sellerId), (0, drizzle_orm_1.between)(b2bOrders_1.b2b_orders.created_at, startDate, endDate), (0, drizzle_orm_1.inArray)(b2bOrders_1.b2b_orders.order_status, billableStatuses))),
        ]);
        allOrders = [...(b2c || []), ...(b2b || [])];
    }
    if (allOrders.length === 0) {
    ...
    await client_1.db
        ...
        .where((0, drizzle_orm_1.eq)(billingInvoices_1.billingInvoices.id, invoiceId));
    console.log(`✅ Invoice ${inv.invoiceNo} regenerated (adjusted total: ${formatAmount(adjustedTotal)}, base: ${formatAmount(stmt.totals.netPayable)})`);
    // Send email notification to user
    try {
        const [user] = await client_1.db.select().from(users_1.users).where((0, drizzle_orm_1.eq)(users_1.users.id, inv.sellerId)).limit(1);
        if (user?.email) {
            const [profile] = await client_1.db
                .select()
                .from(userProfile_1.userProfiles)
                .where((0, drizzle_orm_1.eq)(userProfile_1.userProfiles.userId, inv.sellerId))
                .limit(1);
            const sellerName = profile?.companyInfo?.businessName || user.email.split('@')[0];
            const periodStart = (0, dayjs_1.default)(inv.billingStart).format('DD MMM YYYY');
            const periodEnd = (0, dayjs_1.default)(inv.billingEnd).format('DD MMM YYYY');
            // Get presigned URLs for PDF and CSV
            const signedUrls = await (0, upload_service_1.presignDownload)([inv.pdfUrl, inv.csvUrl]);
            const pdfSignedUrl = Array.isArray(signedUrls) && signedUrls.length > 0 ? signedUrls[0] : undefined;
            const csvSignedUrl = Array.isArray(signedUrls) && signedUrls.length > 1 ? signedUrls[1] : undefined;
            await (0, emailSender_1.sendInvoiceReadyEmail)({
                to: user.email,
                sellerName,
...
exports.regenerateInvoiceWithAdjustments = regenerateInvoiceWithAdjustments;
