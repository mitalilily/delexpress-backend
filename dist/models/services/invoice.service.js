...
exports.getInvoicesService = exports.generateInvoicePDF = void 0;
...
const generateInvoicePDF = async (invoice) => {
    ...
    const invoiceNumber = `${invoice.invoicePrefix ?? ''}${invoice.invoiceNumber}${invoice.invoiceSuffix ?? ''}`;
        ...
        try {
            ...
            if (!isValidImageBuffer(buffer)) {
                ...
                if (buffer.length < 100) {
                    console.warn(`⚠️ Buffer too small (${buffer.length} bytes) - likely corrupted or incomplete download, skipping image`);
                }
                else {
                    console.warn(`⚠️ Could not detect image type and buffer does not appear to be a valid image format (PNG/JPEG/GIF/WebP), skipping image (buffer size: ${buffer.length} bytes)`);
                }
                return null;
            }
            ...
            console.warn(`⚠️ Could not detect image type via file-type, but buffer appears valid. Using ${mimeType}`);
    ...
    try {
        ...
        const logoUrl = await (0, upload_service_1.presignDownload)(logoKey);
        if (logoUrl && typeof logoUrl === 'string') {
            try {
                const resp = await axios_1.default.get(logoUrl, { responseType: 'arraybuffer', timeout: 5000 });
    ...
    const supportLine = invoice.supportEmail && invoice.supportEmail.trim().length > 0
    ...
    const hsnTotalRow = Object.values(hsnSummary).reduce((acc, v) => {
    ...
    const sellerDisplayName = toSafeString(invoice.brandName) || toSafeString(invoice.sellerName) || toSafeString(invoice.companyName);
    ...
    const isInterState = sellerStateCode &&
        buyerStateName &&
    ...
    let sgstTotal = 0;
    let igstTotal = 0;
    const productRows = invoice.products.map((p, index) => {
        const qty = toAmount(p.qty ?? 1);
        const price = toAmount(p.price);
    ...
    const buildClassicLayout = () => {
        ...
        const filteredCharges = chargesBreakdown.filter((item) => {
            if (item.label === 'COD Charges' && codCharges === 0)
                return false;
            if (item.label === 'RTO Charges' && rtoCharges === 0)
                return false;
            return true;
        });
        const chargesTableBody = filteredCharges
        ...
        const gstSummary = gstRows.length > 1 || (gstRows.length === 1 && gstRows[0].value > 0)
            ? {
                table: {
                    widths: ['*', 'auto'],
    ...
    const contentClassic = [buildHeaderBand(), ...buildClassicLayout()];
    // -------------------
    // Thermal Layout
    // -------------------
    const contentThermal = [
    ...
    const normalizeRupeeGlyphs = (node) => {
        if (typeof node === 'string')
            return node.replace(/₹/g, 'Rs.');
        if (Array.isArray(node))
            return node.map(normalizeRupeeGlyphs);
            ...
            pdfDoc.on('error', (err) => {
                ...
                if (err && typeof err === 'object' && 'message' in err) {
                    ...
                    if (errorMsg.includes('Unknown image format') || errorMsg.includes('Invalid image')) {
                        reject(new Error(`Invoice PDF generation failed: Invalid image format. Please check logo/signature files are valid images (PNG, JPEG, GIF, or WebP). Original error: ${errorMsg}`));
                    }
                    else {
...
const getInvoicesService = async ({ page, limit, filters, }) => {
    const offset = (page - 1) * limit;
    const whereClauses = [];
    if (filters.status) {
        whereClauses.push((0, drizzle_orm_1.eq)(invoices_1.invoices.status, filters.status));
    }
    if (filters.userId) {
        whereClauses.push((0, drizzle_orm_1.eq)(invoices_1.invoices.userId, filters.userId));
    }
    if (filters.invoiceNumber) {
        whereClauses.push((0, drizzle_orm_1.ilike)(invoices_1.invoices.invoiceNumber, `%${filters.invoiceNumber}%`));
    }
    if (filters.dateFrom) {
        whereClauses.push((0, drizzle_orm_1.gte)(invoices_1.invoices.invoiceDate, filters.dateFrom));
    }
    if (filters.dateTo) {
        whereClauses.push((0, drizzle_orm_1.lte)(invoices_1.invoices.invoiceDate, filters.dateTo));
    }
    console.log('filters', filters);
    if (filters.awb) {
        // Look into items JSONB for orderId matching AWB
        whereClauses.push((0, drizzle_orm_1.sql) `EXISTS (
        SELECT 1 
        FROM jsonb_array_elements(${invoices_1.invoices.items}) AS item
    ...
    const data = await client_1.db
        ...
        .orderBy((0, drizzle_orm_1.desc)(invoices_1.invoices.invoiceDate))
        .limit(limit)
        .offset(offset);
    // Count total
