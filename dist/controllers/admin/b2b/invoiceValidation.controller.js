...
exports.validateInvoiceFileController = exports.validateInvoiceContentController = void 0;
...
function parseInvoiceData(text) {
    ...
    const gstinMatch = flatText.match(/\b([0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[0-9A-Z]{1}[Z]{1}[0-9A-Z]{1})\b/);
    if (gstinMatch) {
 ...
 */
const validateInvoiceContentController = async (req, res) => {
    try {
        ...
        try {
            ...
            if (fileUrl.includes('http')) {
                ...
                const bucketIndex = urlParts.findIndex((part) => part.includes('r2') || part.includes('s3'));
        ...
        let signedUrl;
        try {
            const presignedResult = await (0, upload_service_1.presignDownload)(fileKey);
        ...
        if (contentType.includes('pdf') || fileKey.toLowerCase().endsWith('.pdf')) {
            ...
            extractedText = await (0, ocr_service_1.extractText)(buffer);
        }
        ...
        if (extractedData.totalValue && invoiceValue) {
            ...
            if (difference > threshold) {
                warnings.push(`Invoice total mismatch: Extracted ₹${extractedData.totalValue.toFixed(2)} vs Provided ₹${invoiceValue.toFixed(2)}`);
            }
        }
...
exports.validateInvoiceContentController = validateInvoiceContentController;
/**
 * Validate invoice file type, size, and dangerous extensions
 */
const validateInvoiceFileController = async (req, res) => {
    try {
        ...
        if (hasDangerousExtension) {
            return res.status(400).json({
                success: false,
                message: 'Dangerous file types (.exe, .js, .php, .html, .zip, .rar, .bat, .sh) are not permitted.',
            });
        }
...
exports.validateInvoiceFileController = validateInvoiceFileController;
