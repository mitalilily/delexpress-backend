...
exports.completeRegistration = exports.getUserById = exports.getCurrentUser = void 0;
...
const getUserById = async (req, res) => {
    try {
        const user = await (0, userService_1.findUserById)(req.params.userId);
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
        return res.status(200).json({
...
const completeRegistration = async (req, res) => {
    ...
    try {
        const user = await (0, userService_1.findUserById)(userId);
        const userProfile = await (0, userProfile_service_1.getProfileByUserId)(userId);
        if (!user)
            return res.status(404).json({ error: 'User not found' });
        let updates = {};
        const isOnlyB2B = Array.isArray(data?.businessLegal?.businessCategory) &&
        ...
        switch (step) {
            ...
            case 1: {
                ...
                /* --- Uniqueness checks --- */
                if (phoneDigits) {
                    const other = await (0, userService_1.findUserByPhone)(phoneDigits);
                ...
                if (emailLower) {
                    const other = await (0, userService_1.findUserByEmail)(emailLower);
                    if (other && other.id !== userId) {
                        return res.status(400).json({
        ...
        const updatedUser = await (0, userProfile_service_1.upsertUserProfile)(userId, updates);
