...
exports.delhiveryWebhookHandler = exports.delhiveryDocumentPushHandler = exports.delhiveryScanPushHandler = void 0;
 ...
 */
const delhiveryScanPushHandler = async (req, res) => {
    ...
    try {
        ...
        const result = await (0, webhookProcessor_1.processDelhiveryWebhook)(payload);
        ...
        if (!result.success && result.reason === 'order_not_found') {
            ...
            const [existingPending] = await client_1.db
                ...
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.pending_webhooks.awb_number, String(awb)), (0, drizzle_orm_1.eq)(schema_1.pending_webhooks.status, String(status || 'unknown')), (0, drizzle_orm_1.isNull)(schema_1.pending_webhooks.processed_at), (0, drizzle_orm_1.gte)(schema_1.pending_webhooks.created_at, dedupeWindowStart)))
                .limit(1);
            if (!existingPending) {
                ...
                console.warn(`⚠️ Stored Delhivery scan push webhook for AWB ${awb} (order not yet created).`);
            }
            else {
        ...
        console.warn(`⚠️ Delhivery scan push webhook partially processed for AWB: ${awb}, reason: ${result.reason}`);
        return res.status(202).json({ success: false });
    }
    catch (err) {
        console.error('='.repeat(80));
        console.error(`❌ [${timestamp}] Delhivery scan push webhook error for AWB: ${awb || 'unknown'}`);
...
exports.delhiveryScanPushHandler = delhiveryScanPushHandler;
/**
 * Delhivery Document Push Webhook Handler
 * Handles POD, Sorter Image, and QC Image document pushes
 */
const delhiveryDocumentPushHandler = async (req, res) => {
    const timestamp = new Date().toISOString();
    const payload = req.body;
    ...
    try {
        ...
        console.log(`📄 Processing Delhivery document push webhook for AWB: ${awb}, Type: ${documentType}`);
        const result = await (0, webhookProcessor_1.processDelhiveryDocumentWebhook)(payload, documentType);
        if (result.success) {
        ...
        console.error('='.repeat(80));
        console.error(`❌ [${timestamp}] Delhivery document push webhook error for AWB: ${awb || 'unknown'}`);
        console.error(`   Error Message: ${err?.message || err}`);
        console.error(`   Error Stack:`, err?.stack);
        console.error(`   Payload:`, JSON.stringify(payload, null, 2));
        console.error('='.repeat(80));
...
exports.delhiveryDocumentPushHandler = delhiveryDocumentPushHandler;
/**
 ...
 */
const delhiveryWebhookHandler = async (req, res) => {
    const timestamp = new Date().toISOString();
    const payload = req.body;
    const shipment = payload?.Shipment;
    const awb = shipment?.AWB || payload?.waybill || payload?.AWB || null;
    ...
    const isDocumentPush = payload?.DocumentType ||
        payload?.PODDocument ||
        payload?.SorterImage ||
        payload?.QCImage ||
    ...
    console.log(`   Type: ${isDocumentPush ? `Document Push (${documentType})` : 'Scan Push (Status Update)'}`);
    console.log(`   AWB: ${awb || 'N/A'}`);
    console.log(`   Status: ${status}`);
    console.log(`   IP: ${req.ip || req.socket.remoteAddress || 'unknown'}`);
    console.log(`   Headers:`, JSON.stringify(req.headers, null, 2));
    ...
    try {
        ...
        // Handle known soft errors (e.g. invalid status)
        console.warn(`⚠️ Delhivery webhook partially processed for AWB: ${awb}, reason: ${result.reason}`);
        return res.status(202).json({ success: false });
    }
    catch (err) {
        console.error('='.repeat(80));
...
exports.delhiveryWebhookHandler = delhiveryWebhookHandler;
