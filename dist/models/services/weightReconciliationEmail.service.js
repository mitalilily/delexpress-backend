...
exports.sendWeightDiscrepancyEmail = sendWeightDiscrepancyEmail;
exports.sendDailySummaryEmail = sendDailySummaryEmail;
exports.sendDisputeUpdateEmail = sendDisputeUpdateEmail;
 ...
 */
async function sendWeightDiscrepancyEmail(data) {
    const { userEmail, userName, orderNumber, awbNumber, courierPartner, declaredWeight, chargedWeight, weightDifference, additionalCharge, discrepancyId, autoAccepted = false, autoAcceptThreshold, } = data;
    ...
    const html = `
      ...
      <head>
        <style>
          ...
          .alert { background: ${autoAccepted ? '#d4edda' : isLargeDiscrepancy ? '#fff3cd' : '#d1ecf1'}; border-left: 4px solid ${autoAccepted ? '#28a745' : isLargeDiscrepancy ? '#ffc107' : '#0dcaf0'}; padding: 15px; margin: 20px 0; }
          ...
          .value { color: #333; }
          .highlight { color: ${weightDifference > 0 ? '#E74C3C' : '#27AE60'}; font-weight: bold; font-size: 18px; }
      ...
      <body>
        <div class="container">
          <div class="header">
            <h2 style="margin: 0;">${autoAccepted ? '✅ Weight Discrepancy Auto-Accepted' : '⚖️ Weight Discrepancy Alert'}</h2>
          </div>
        ...
        ? `<div class="alert">
                  ...
                  <div class="auto-accept-info">
                    ...
                    <ul style="margin: 10px 0; padding-left: 20px;">
                      <li><strong>Discrepancy Accepted:</strong> The weight difference (${Math.abs(weightDifference).toFixed(3)} kg) was within your auto-acceptance threshold${autoAcceptThreshold ? ` (${autoAcceptThreshold} kg)` : ''}</li>
                      <li><strong>Order Updated:</strong> The order's shipping charge has been updated to reflect the actual weight</li>
                      ${additionalCharge > 0
            ? `<li><strong>Wallet Charge Applied:</strong> ₹${additionalCharge.toFixed(2)} has been automatically deducted from your wallet balance</li>`
            : ''}
        ...
        : ''}
              ...
              <div class="detail-row">
                ...
                <span class="highlight">${weightDifference > 0 ? '+' : ''}${weightDifference.toFixed(3)} kg</span>
              </div>
        ...
        ? `
            ...
            <center>
              <a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/reconciliation/weight/${discrepancyId}" class="button">
                Review Discrepancy
              </a>
            </center>
 ...
 */
async function sendDailySummaryEmail(data) {
    const { userEmail, userName, date, totalDiscrepancies, pendingCount, acceptedCount, disputedCount, totalAdditionalCharges, discrepancies, } = data;
      ...
      <body>
        ...
        ? `
            ...
            .map((d) => `
                    <tr>
                      <td>${d.orderNumber}</td>
                      <td>${d.weightDifference > 0 ? '+' : ''}${d.weightDifference.toFixed(3)} kg</td>
                      <td>₹${d.additionalCharge.toFixed(2)}</td>
                      <td><span style="color: ${d.status === 'pending'
            ? '#F39C12'
            : d.status === 'accepted'
                ? '#27AE60'
                : '#3498DB'};">${d.status.toUpperCase()}</span></td>
                    </tr>
                  `)
            .join('')}
                </tbody>
        ...
        : '<p>No discrepancies detected today! 🎉</p>'}
            ...
            <center>
              <a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/reconciliation/weight" class="button">
                View Full Report
              </a>
            </center>
          </div>
          ...
          <div class="footer">
            ...
            <p>You can manage email preferences in your <a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/reconciliation/weight/settings">settings</a></p>
            <p>© ${new Date().getFullYear()} DelExpress. All rights reserved.</p>
          </div>
        </div>
      </body>
 ...
 */
async function sendDisputeUpdateEmail(userEmail, userName, orderNumber, disputeStatus, adminComment) {
    const statusColors = {
        approved: '#27AE60',
        rejected: '#E74C3C',
        under_review: '#3498DB',
    ...
    const html = `
      ...
      <head>
        <style>
          ...
          .header { background: #333369; color: white; padding: 20px; border-radius: 8px 8px 0 0; }
          .content { background: #f9f9f9; padding: 30px; border: 1px solid #ddd; }
          .status-badge { display: inline-block; padding: 8px 16px; border-radius: 5px; font-weight: bold; color: white; background: ${statusColors[disputeStatus] || '#6B7280'}; }
...
async function sendWeeklyReportEmail(data) {
    const { userEmail, userName, weekStart, weekEnd, totalDiscrepancies, pendingCount, acceptedCount, disputedCount, resolvedCount, rejectedCount, totalAdditionalCharges, avgWeightDifference, maxWeightDifference, autoAcceptedCount, courierBreakdown, topDiscrepancies, } = data;
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="highlight-box">
              ...
              <p><strong>Total Additional Charges:</strong> ₹${totalAdditionalCharges.toFixed(2)}</p>
              <p><strong>Average Weight Difference:</strong> ${avgWeightDifference > 0 ? '+' : ''}${avgWeightDifference.toFixed(3)} kg</p>
              <p><strong>Maximum Weight Difference:</strong> ${maxWeightDifference > 0 ? '+' : ''}${maxWeightDifference.toFixed(3)} kg</p>
