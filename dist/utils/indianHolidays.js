...
exports.fetchIndianHolidaysFromAPI = exports.getIndianNationalHolidays = void 0;
/**
 ...
 */
const getIndianNationalHolidays = (year) => {
    const holidays = [];
...
exports.getIndianNationalHolidays = getIndianNationalHolidays;
/**
 ...
 */
const fetchIndianHolidaysFromAPI = async (year) => {
    let apiHolidays = [];
            ...
            if (contentType && contentType.includes('application/json')) {
                ...
                if (Array.isArray(data) && data.length > 0) {
                    ...
                    apiHolidays = data.map((holiday) => {
                        ...
                        const isRecurring = recurringHolidays.some((name) => holiday.name.toLowerCase().includes(name.toLowerCase()));
                        return {
                            name: holiday.name,
                            date: holiday.date,
                            isRecurring,
    ...
    const calculatedHolidays = (0, exports.getIndianNationalHolidays)(year);
    ...
    if (apiHolidays.length > 0) {
        ...
        console.log(`📅 Merged ${apiHolidays.length} holidays (${apiHolidays.length - calculatedHolidays.length} from API + regional holidays)`);
        return apiHolidays;
    }
    // Fallback to calculated dates only (includes all regional holidays like Lohri, Pongal, etc.)
    console.log(`📅 Using calculated dates for ${year} (includes regional holidays like Lohri, Pongal, Onam, etc.)`);
    return calculatedHolidays;
};
exports.fetchIndianHolidaysFromAPI = fetchIndianHolidaysFromAPI;
