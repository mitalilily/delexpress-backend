...
exports.XpressbeesService = void 0;
...
class XpressbeesService {
    constructor() {
        this.baseApi = process.env.XPRESSBEES_API_BASE || 'https://shipment.xpressbees.com';
        this.apiToken = process.env.XPRESSBEES_API_TOKEN || '';
        this.username = process.env.XPRESSBEES_USERNAME || '';
        this.password = process.env.XPRESSBEES_PASSWORD || '';
        this.tokenEndpoint = process.env.XPRESSBEES_TOKEN_ENDPOINT || '/api/users/login';
        this.shipmentEndpoint = process.env.XPRESSBEES_SHIPMENT_ENDPOINT || '/api/shipments2';
        this.reverseShipmentEndpoint = process.env.XPRESSBEES_REVERSE_SHIPMENT_ENDPOINT || '/api/reverseshipments';
    }
    ...
    async ensureConfigLoaded() {
        if (XpressbeesService.cachedConfig === undefined) {
            XpressbeesService.cachedConfig = await (0, courierCredentials_service_1.getEffectiveCourierConfig)('xpressbees', 'b2c');
        }
    ...
    buildEndpointForBase(baseApi, path) {
    ...
    getBaseCandidates() {
        ...
        const rawEnvAlternates = String(process.env.XPRESSBEES_ALT_API_BASE || '')
            ...
            .map((item) => item.trim())
            .filter(Boolean);
        for (const alt of rawEnvAlternates)
            push(alt);
        const currentBaseWithoutApi = this.baseApi.replace(/\/api$/i, '');
        if (/shipment\.xpressbees\.com/i.test(currentBaseWithoutApi)) {
            push(currentBaseWithoutApi.replace(/shipment\.xpressbees\.com/i, 'xbclientapi.xpressbees.com'));
            push(`${currentBaseWithoutApi.replace(/shipment\.xpressbees\.com/i, 'xbclientapi.xpressbees.com')}/api`);
        }
        if (/xbclientapi\.xpressbees\.com/i.test(currentBaseWithoutApi)) {
            push(currentBaseWithoutApi.replace(/xbclientapi\.xpressbees\.com/i, 'shipment.xpressbees.com'));
            push(`${currentBaseWithoutApi.replace(/xbclientapi\.xpressbees\.com/i, 'shipment.xpressbees.com')}/api`);
        }
    ...
    async requestWithFallback({ method, pathCandidates, data, logPayload = true, }) {
        ...
        const allAttemptsWereEndpointMisses = attemptedUrls.length > 0 &&
            this.isRetryableEndpointError(lastError) &&
            attemptedUrls.length >= dedupedPaths.length;
        if (allAttemptsWereEndpointMisses) {
            throw new classes_1.HttpError(502, ` API endpoint not found for the configured base URL. Tried: ${attemptedUrls.join(', ')}. Verify the  API base URL in courier credentials and confirm your account's shipment endpoint.`);
        }
        if (lastError) {
            throw new classes_1.HttpError(Number(lastError?.response?.status || 502), this.extractErrorMessage(lastError, 'Xpressbees API request failed'));
        }
        throw lastError;
    }
    async getHttp() {
    ...
    async persistGeneratedToken(token) {
        try {
            await client_1.db
    ...
    async generateApiToken() {
        ...
        if (!this.username || !this.password) {
            throw new Error('Xpressbees API token is not configured and username/password are missing. Save bearer token or login credentials in courier credentials.');
        }
        const loginHttp = axios_1.default.create({
        ...
        throw new Error(`Failed to generate Xpressbees API token. Tried ${endpointCandidates.join(', ')}. ${lastError?.response?.data?.message || lastError?.message || 'unknown error'}. Set XPRESSBEES_TOKEN_ENDPOINT if your account uses a different login path.`);
    }
    async getApiToken(forceRefresh = false) {
        await this.ensureConfigLoaded();
        if (!forceRefresh && this.apiToken)
            return this.apiToken;
        return this.generateApiToken();
    }
    async checkServiceability(payload) {
        const raw = await this.requestWithFallback({
            method: 'post',
        ...
        return {
            ...
            records,
            codAvailable: payload.payment_type === 'prepaid'
                ? true
                : records.some((record) => Number(record?.cod_charges ?? 0) >= 0),
    ...
    async createShipment(payload) {
        const body = {
            ...
            request_auto_pickup: String(payload.request_auto_pickup || '').toLowerCase() === 'yes' ? 'yes' : 'no',
            consignee: {
            ...
            collectable_amount: payload?.payment_type === 'cod'
    ...
    async createReverseShipment(payload) {
        const body = {
...
exports.XpressbeesService = XpressbeesService;
